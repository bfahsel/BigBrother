...
class URLProtocolTests: XCTestCase {
    ...
    fileprivate func swizzleRegisterClass() {
        let method: Method = class_getClassMethod(object_getClass(URLProtocol), #selector(URLProtocol.registerClass(_:)))
        let swizzledMethod: Method = class_getClassMethod(object_getClass(URLProtocol), #selector(URLProtocol.bb_registerClass(_:)))
    ...
    fileprivate func swizzleUnregisterClass() {
        let method: Method = class_getClassMethod(object_getClass(URLProtocol), #selector(URLProtocol.unregisterClass(_:)))
        let swizzledMethod: Method = class_getClassMethod(object_getClass(URLProtocol), #selector(URLProtocol.bb_unregisterClass(_:)))
    ...
    override func tearDown() {
        ...
        URLProtocol.unregisterClass(BigBrother.URLProtocol)
        URLProtocol.registeredClasses = NSMutableArray()
    ...
    func testAddToSharedSession() {
        let previousNumberOfProtocols = URLProtocol.registeredClasses.count
        
        BigBrother.addToSharedSession()
        
        let numberOfProtocols = URLProtocol.registeredClasses.count
        XCTAssertEqual(numberOfProtocols, previousNumberOfProtocols + 1)
        
        XCTAssertTrue(URLProtocol.registeredClasses.contains { $0 === BigBrother.URLProtocol.self } )
    }
    ...
    func testRemoveFromSharedSession() {
        ...
        XCTAssertFalse(URLProtocol.registeredClasses.contains { $0 === BigBrother.URLProtocol.self } )
    }
}
